...
z = [1,2]
for i in range(2,red+bit):
    z.append(m.Var(1,lb=1,ub=2**(bit+red+1),integer=True))
print(len(z), bit+red)
# Dummy column
...
m.Equation(sum_x<=2**(bit+2))
temp = 1    
for i in range(2,bit+red):
    m.Equation(x_sum[i-1]-x[i]>0)
...
for item in z:
    if hasattr(item,'value'):
        ...
        if c < setup['column_bit']:
            col_ary.append(int(item.value[-1]))
        elif c == col:
            reference_bit_size = int(item.value[-1])
        else:
            row_ary.append(int(item.value[-1]))
    else:
        solution.append(item)
        if c < setup['column_bit']:
            col_ary.append(int(item))
        elif c == col:
            reference_bit_size = int(item)
        else:
            row_ary.append(int(item))
    c = c + 1
if bit > 9:
...
t = 1
for i in range(bit+red):
    sol_bits.append(int(solution[i]))
    t = t + solution[i]
diff = []
