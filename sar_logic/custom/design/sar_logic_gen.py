...
class sar_logic:
    ...
    def add_fixed_inst(self):
        ...
        self.items.append( instance(self.cnt+15, 0 , "nor2", {"ZN":"net010", "A1":"cmp_p", "A2":"cmp_n"}).inst )
        self.items.append( instance(self.cnt+16, 1 , "delay", {"Z":"net08", "I":"net010"}).inst )
        self.cnt += 17
    ...
    def add_sized_inst(self):
        ...
        if buffer_size['clkc'] <= 4:
            self.items.append( instance(self.cnt, buffer_size['clkc'] , "nand2", {"ZN":"clkc", "A1":"Q["+str(self.bit+2)+"]", "A2":"clock"}).inst )
            self.cnt += 1
        else:
            self.items.append( instance(self.cnt, 4, "and2", {"Z":"clkc_b", "A1":"Q["+str(self.bit+2)+"]", "A2":"clock"}).inst )
            self.items.append( instance(self.cnt+1, buffer_size['clkc'] , "inv", {"ZN":"clkc", "I":"clkc_b"}).inst )
            self.cnt += 2
